...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let leftBracket = '[';
  let rightBracket = ']';
  if (!isStartIncluded) leftBracket = '(';
  if (!isEndIncluded) rightBracket = ')';
  if (a > b) {
...
function isBracketsBalanced(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    const bracket = str[i];
    if (pairsOfBrackets[bracket]) {
      if (stackOfBrackets.length === 0 || stackOfBrackets.pop() !== pairsOfBrackets[bracket]) {
        return false;
      }
    } else {
...
function getCommonDirectoryPath(pathes) {
  let shortPath = pathes[0];
  let prefix = '';
  for (let i = 0; i < pathes.length; i += 1) {
    if (pathes[i].length < shortPath.length) {
      shortPath = pathes[i];
    }
  }
  for (let i = 0; i < shortPath.length; i += 1) {
    if (pathes.every((element) => element[i] === shortPath[i])) {
      prefix += shortPath[i];
    } else {
      break;
    }
  }
  const indexOfLastSlash = prefix.lastIndexOf('/');
  if (indexOfLastSlash === -1) {
    return '';
  }
  return prefix.slice(0, indexOfLastSlash + 1);
}
...
function getMatrixProduct(m1, m2) {
  const result = new Array(m1.length).fill(null).map(() => new Array(m2[0].length).fill(0));
...
function evaluateTicTacToePosition(position) {
  for (let i = 0; i < 3; i += 1) {
    const hack = position[i][2];
    if (position[i][0] === position[i][1] && position[i][1] === hack && hack !== undefined) {
      return position[i][0];
    }
